...
const getWordTokens = _.memoize((texts) =>
  _.map(texts, (text) => splitText(text)),
)

const getWordTokenIndex = _.memoize((texts) =>
  _.map(getWordTokens(texts), (wordTokens) =>
    _.map(wordTokens, (word) => _.toNumber(_.get(getWordIndex(texts), word))),
  ),
)
